...
var Graphite = null; // loaded on demand
...
var internals = {
    host: Os.hostname(),
    appVer: require(Path.normalize(process.cwd() + '/package.json')).version || 'unknown',
};
...
internals.defaults = {
    ...
    graphitePrefix: 'hapi.ops'                      // Prefix for graphite metrics
};
...
internals.Monitor.prototype._broadcastGraphite = function () {
    var self = this;
    var request = function (uri, metric) {
        // load on demand
        Graphite = Graphite || require('graphite');

        var url = 'plaintext://' + uri.hostname + ':' + uri.port;
        var client = Graphite.createClient(url);
        client.write(metric, function() {
            client.end();
        });
    };

    var flatten = function(obj, results, path, ignoreList) {
        results = results || [];
        Object.keys(obj).forEach(function(key) {
            var oldPath = path + '.';
            if(ignoreList.indexOf(oldPath + key) === -1) {
                var val = obj[key];
                ...
                if (typeof val != 'object') {
                  ...
                  // to the recursion mobile!
                  flatten(val, results, newPath, ignoreList);
                }
            }
        });
        return results;
    ...
    Object.keys(self._subscriberQueues.graphite).forEach(function (uri) {
        var subscriberQueue = self._subscriberQueues.graphite[uri];
        if (!subscriberQueue.length) {
            return;
        }
        var events = self._eventsFilter(self._subscriberTags[uri], subscriberQueue);

        var uri_obj = Url.parse(uri);
        events.forEach(function(event) {
            var ignoreList = ['hapi.ops.event'];
            var flatEvents = flatten(event, [], self.settings.graphitePrefix, ignoreList);
            flatEvents.forEach(function(metric) {
              request(uri_obj, metric);
            });
        });
